...
import analyze_smp_flat_objective_alignment as analysis
...
class FlatObjectiveAnalysisTest(unittest.TestCase):
  def _fixture(self, root: Path, failed_seed: int | None = None) -> analysis.FlatObjectiveAnalysisCfg:
    ...
    return analysis.FlatObjectiveAnalysisCfg(
  ...
  def test_complete_negative_result_is_retained_without_promotion(self) -> None:
    with tempfile.TemporaryDirectory() as temporary:
      cfg = self._fixture(Path(temporary), failed_seed=20261103)
      result = analysis.write_analysis(cfg)
      self.assertEqual(result["status"], "FLAT_OBJECTIVE_COMPLETE_NO_PROMOTION")
  ...
  def test_failure_telemetry_mismatch_fails_closed(self) -> None:
    with tempfile.TemporaryDirectory() as temporary:
      ...
      raw = next(cfg.evaluation_root.glob("**/a9_objective_aligned__*.json"))
