...
import build_smp_flat_objective_alignment_manifests as builder


class FlatObjectiveManifestTest(unittest.TestCase):
  def _fixture(self, root: Path) -> builder.FlatObjectiveManifestCfg:
    protocol = Path(__file__).parents[1] / "docs" / "ral_flat_objective_alignment_v1.json"
    jobs = []
    for gpu, (arm, seed) in enumerate(
      ...
      objective = (
        ...
        if arm == "a6_replication_control"
          ...
          "      smp_floor: 0.35\n"
          "      recovery_initiation_progress: true\n"
          "      track_head_height: true\n"
          "      upright_posture: true\n"
          "      feet_stationary_when_upright: true\n"
          "      base_stationary_when_upright: true\n"
          "      stable_stand_metric: true\n"
      ...
      stood_up = (
        ...
        if arm == "a9_objective_aligned" else ""
      )
    ...
    return builder.FlatObjectiveManifestCfg(
  ...
  def test_index_detects_tampering_and_partial_artifacts(self) -> None:
    ...
    with tempfile.TemporaryDirectory() as temporary:
      ...
      (cfg.output_dir / "gate_8000_seed_20261101.json").write_text("{}")
