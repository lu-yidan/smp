"""Audit and summarize the frozen V38 recovery-route evaluation."""
...
from run_smp_v38_recovery_route_eval import _read_rows
...
def _bootstrap_final(effects: list[dict[str, Any]]) -> dict[str, Any]:
  final = [row for row in effects if row["gate"] == 7999]
  rng = random.Random(20262110)
...
def main() -> None:
  ...
  parser.add_argument(
    "--control-dir",
    type=Path,
    default=Path("run_control/v38_93d_recovery_route_v1/evaluation"),
  )
  args = parser.parse_args()
  plan = json.loads((args.control_dir / "immutable_plan.json").read_text())
  summaries = [_summarize(job) for job in plan["jobs"]]
  if len(summaries) != 36 or sum(row["cells"] for row in summaries) != 216:
    raise RuntimeError("V38_EVAL_ALERT: expected 36 matrices and 216 cells")
  effects = _paired(summaries)
  result = {
    "schema_version": 1,
    "status": "V38_DIAGNOSTIC_COMPLETE_NO_AUTOMATIC_HARDWARE_AUTHORIZATION",
  ...
  final = [row for row in summaries if row["gate"] == 7999]
  lines = [
    "# V38 recovery-route diagnostic",
    "",
