"""Prepare an RSL-RL checkpoint with a fixed exploration standard deviation."""
...
def main(cfg: ConvertCfg) -> None:
  ...
  if cfg.output.exists():
    raise FileExistsError(f"Refusing to overwrite checkpoint: {cfg.output}")
  ...
  print(f"source_sha256={infos['fixed_std_seed']['source_sha256']}")
