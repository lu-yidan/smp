"""Build immutable four-gate manifests for the fresh-seed flat objective-alignment study."""
...
_PLAN_ID = "83907a6188a983184ba255f3224dad89ff250c1afc5137caa413951f7b119af8"
_LAUNCH_COMMIT = "8855446adae35ae56a6a7b667671010b8199ac09"
_SEEDS = (20261101, 20261102, 20261103)
...
_ARMS = {
  ...
  "a9_objective_aligned": {
    "task": "Smp-Getup-Scratch-A9-F2S2-Objective-Aligned-G1",
    "experiment": "smp_scratch_a9_f2s2_objective_aligned_g1",
    "procedural_probability": 0.20,
...
class FlatObjectiveManifestCfg:
  launch_manifest: Path = Path(
    "run_control/flat_objective_alignment_v1_training/launch_manifest.json"
  )
  output_dir: Path = Path("run_control/flat_objective_alignment_v1_eval/manifests")
...
def _validate_launch(cfg: FlatObjectiveManifestCfg) -> tuple[dict[str, Any], dict[str, Any]]:
  launch = _load(cfg.launch_manifest)
  protocol = _load(cfg.protocol)
  if _sha256(cfg.protocol) != _PROTOCOL_SHA256:
    raise ValueError("flat objective alignment protocol SHA-256 drifted")
  if protocol.get("status") != "PREREGISTERED_READY_FOR_TRAINING":
    raise ValueError("flat objective alignment protocol status drifted")
  if (
    ...
    raise ValueError("flat objective alignment launch lineage drifted")
  if Path(launch.get("protocol", "")).resolve() != cfg.protocol.resolve():
    raise ValueError("flat objective alignment launch protocol path drifted")
  expected_top = {
  ...
  for name, expected in expected_top.items():
    if launch.get(name) != expected:
      raise ValueError(f"flat objective alignment launch {name} drifted")
  smoke = launch.get("implementation_smoke", {})
  if smoke.get("status") != "PASSED_REAL_MUJOCO_SMOKE":
    raise ValueError("flat objective alignment launch lacks the frozen smoke audit")
  preflight = launch.get("resource_preflight", {})
  if preflight.get("free_gib", 0) < 100 or preflight.get("inode_free_fraction", 0) < 0.10:
    raise ValueError("flat objective alignment launch resource preflight is invalid")
  jobs = launch.get("jobs")
  if not isinstance(jobs, list) or len(jobs) != 6:
    raise ValueError("flat objective alignment launch must contain six jobs")
  ...
  if observed_pairs != expected_pairs:
    raise ValueError("flat objective alignment launch does not contain the full arm/seed factorial")
  expected_order = [(arm, seed) for arm in _ARMS for seed in _SEEDS]
  if [(job["arm"], int(job["policy_seed"])) for job in jobs] != expected_order:
    raise ValueError("flat objective alignment launch job order drifted")
  for gpu, job in enumerate(jobs):
    ...
    if (
      ...
      raise ValueError(f"flat objective alignment launch job drifted: {arm}/{seed}")
...
def _verify_saved_config(run_dir: Path, job: dict[str, Any]) -> None:
  ...
  if arm == "a6_replication_control":
    if "procedural_smp_floor: 0.1" not in reward:
      raise ValueError(f"saved A6 reward bridge drifted: {run_dir}")
  else:
    ...
    if any(term not in reward for term in required_objective_terms):
      raise ValueError(f"saved A9 objective alignment drifted: {run_dir}")
    for term in ("head_vertical_overspeed", "action_rate_l2"):
      if term not in text:
        raise ValueError(f"saved A9 objective penalty drifted: {run_dir}")
    stood_up = _named_block(text, "terminations", "stood_up")
    for term in (
      "head_height: 1.1",
      "max_speed: 0.5",
      "hold_steps: 25",
      ...
      if term not in stood_up:
        raise ValueError(f"saved A9 strict stood-up terminal drifted: {run_dir}")
...
def build(
  ...
  if tuple(cfg.expected_seeds) != _SEEDS or tuple(cfg.checkpoint_steps) != _GATES:
    raise ValueError("flat objective alignment seed or checkpoint gates differ from the frozen protocol")
  ...
  if missing:
    raise FileNotFoundError(
      "refusing partial flat objective alignment manifests; missing:\n"
...
def write_manifests(cfg: FlatObjectiveManifestCfg) -> dict[str, Any]:
  ...
  if existing_index is not None and existing_index.get("index_id") != index["index_id"]:
    raise ValueError(f"existing flat objective alignment index conflicts: {index_path}")
  manifest_paths = set(cfg.output_dir.glob("gate_*_seed_*.json"))
  if existing_index is None and manifest_paths:
    raise ValueError("partial flat objective alignment manifests exist without an index")
  existing_rows = {
  ...
  for (seed, gate), payload in sorted(payloads.items()):
    ...
    if path.exists():
      ...
      if existing.get("manifest_id") != payload["manifest_id"]:
        raise ValueError(f"existing flat objective alignment manifest conflicts: {path}")
      if (
        frozen is None
        or frozen.get("path") != str(path.resolve())
        or frozen.get("sha256") != _sha256(path)
      ):
        raise ValueError(f"flat objective alignment manifest changed after indexing: {path}")
    else:
      if existing_index is not None:
        raise ValueError(f"indexed flat objective alignment manifest is missing: {path}")
  ...
  if existing_index is not None:
    if existing_index.get("manifests") != rows:
      raise ValueError(f"existing flat objective alignment index rows conflict: {index_path}")
  else:
    _atomic_json(index_path, index)
  return index


def main(cfg: FlatObjectiveManifestCfg) -> None:
...
if __name__ == "__main__":
  main(tyro.cli(FlatObjectiveManifestCfg))
