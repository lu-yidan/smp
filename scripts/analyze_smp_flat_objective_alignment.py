"""Audit and aggregate the frozen fresh-seed flat objective-alignment study."""
...
_STUDY_ID = "smp-flat-objective-alignment-v1"
_ARMS = ("a6_replication_control", "a9_objective_aligned")
_PROPOSED = "a9_objective_aligned"
...
class FlatObjectiveAnalysisCfg:
  manifest_index: Path = Path(
    "run_control/flat_objective_alignment_v1_eval/manifests/index.json"
  )
...
def _validate_protocol(path: Path) -> dict[str, Any]:
  ...
  if (
    ...
    raise ValueError("flat objective alignment protocol identity drifted")
  return protocol
...
def _validate_index(path: Path) -> tuple[dict[str, Any], dict[tuple[int, int], dict[str, Any]]]:
  ...
  if (
    ...
    raise ValueError("flat objective alignment manifest index drifted")
  stable_index = {
  ...
  if index.get("index_id") != expected_index_id:
    raise ValueError("flat objective alignment manifest index ID drifted")
  rows = index.get("manifests")
  if not isinstance(rows, list) or len(rows) != 12:
    raise ValueError("flat objective alignment manifest index must contain twelve rows")
  ...
  for row in rows:
    ...
    if key in by_key or not manifest_path.is_file() or _sha256(manifest_path) != row.get("sha256"):
      raise ValueError(f"flat objective alignment manifest index row changed: {key}")
    by_key[key] = row
  if set(by_key) != {(seed, gate) for seed in _SEEDS for gate in _GATES}:
    raise ValueError("flat objective alignment manifest index factorial is incomplete")
  return index, by_key
...
def _audit_matrix(
  ...
  if (
    ...
    raise ValueError(f"flat objective alignment manifest identity drifted: seed {seed} gate {gate}")
  stable_manifest = {
  ...
  if manifest.get("manifest_id") != hashlib.sha256(
    ...
    raise ValueError(f"flat objective alignment manifest ID drifted: seed {seed} gate {gate}")
  manifest_runs = manifest.get("runs")
  if not isinstance(manifest_runs, list) or len(manifest_runs) != 2:
    raise ValueError("flat objective alignment manifest must contain both arms")
  runs = {str(run["name"]): run for run in manifest_runs}
  if set(runs) != set(_ARMS):
    raise ValueError("flat objective alignment manifest arm set drifted")
...
def analyze(cfg: FlatObjectiveAnalysisCfg) -> dict[str, Any]:
  ...
  return {
    "schema_version": 1,
    "status": "PROMOTE_TP_SPECIALISTS" if promote else "FLAT_OBJECTIVE_COMPLETE_NO_PROMOTION",
    "study_id": _STUDY_ID,
    "claim_boundary": (
      "This fresh-seed flat study can only promote A9 into the existing T/P specialist "
...
def _markdown(result: dict[str, Any]) -> str:
  lines = [
    "# Fresh-seed flat objective alignment study",
    "",
  ...
  lines.extend(("", "## Paired final effects (A9 - A6)", ""))
...
def _write_immutable(path: Path, content: str) -> None:
  if path.exists():
    if path.read_text() != content:
      raise ValueError(f"existing flat objective alignment analysis conflicts: {path}")
    return
...
def write_analysis(cfg: FlatObjectiveAnalysisCfg) -> dict[str, Any]:
  result = analyze(cfg)
...
def main(cfg: FlatObjectiveAnalysisCfg) -> None:
...
if __name__ == "__main__":
  main(tyro.cli(FlatObjectiveAnalysisCfg))
